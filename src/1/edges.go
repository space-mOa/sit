		...
		*/
		...
		for _, n2V := range n2.values { // Vyber vrchol ze skupiny vrcholů, n2
			...
			for _, link := range n2V.links {        // Projdi všechny odkazy pro daný vrchol
				if modifyString(n1Title) == modifyString(link) { // Odstraní mezery a transformuje všechna písmenka na malá
...
/*
func walk(x Node, y Node, i uint64) {
	xy := append(x, y)
	for _, v1 := range x.values {
		title1 := v1.line[1]
...
func (e *Edge) isThere(T1, T2 string) (found bool) {
	...
	for _, L := range e.line {
		if modifyString(L[3]) == modifyString(T1) && modifyString(L[4]) == modifyString(T2) {
			found = true
		}
		if modifyString(L[3]) == modifyString(T2) && modifyString(L[4]) == modifyString(T1) {
			found = true
		}
	}
	return found
...
func modifyString(s string) (n string) {

        ...
        d3.csv("./data/edges/Sociologove_Sociologove.csv").then(soc_soc => {
            d3.csv("./data/edges/Sociologove_Casopisy.csv").then(soc_cas => {
                d3.csv("./data/edges/Casopisy_Casopisy.csv").then(cas_cas => {
                    draw({
                        nodes: giveIDs([soc, cas], ["sociolog", "casopis"]), 
                        edges: makeEdges(giveIDs([soc, cas], ["sociolog", "casopis"]), [soc_cas,cas_cas])
                    })
                })
            })
        })
...
function draw(data) {    
    ...
    let simulation = d3.forceSimulation(data.nodes)
        ...
        .force("colide", d3.forceCollide(r + 5))
    ...
    let labels = gc
            .append("text")
                .text(d => d.name)
                .attr("fill", d => (d.nodeName === "sociolog") ? "#87082a" : "#0d8aff")
